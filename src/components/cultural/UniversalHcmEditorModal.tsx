...
const ImageInputWithPreview: React.FC<{
  ...
  placeholder?: string;
}> = ({ label, value, onChange, placeholder = 'Dán đường dẫn URL ảnh (Google Drive, Cloudinary, Web...) hoặc Tải ảnh từ máy' }) => {
    ...
    <div className="space-y-2 p-3.5 rounded-2xl bg-rose-50/80 border border-rose-200/90 shadow-2xs">
      <label className="block text-xs font-bold text-rose-950 flex items-center justify-between">
        ...
        {value && (
          <button
            type="button"
            onClick={() => onChange('')}
            className="text-[10px] text-rose-700 hover:text-rose-900 hover:underline flex items-center gap-1 font-semibold cursor-pointer"
          >
      ...
      <div className="flex items-center gap-2">
        <input
          ...
          className="flex-1 px-3 py-2 border border-rose-200 rounded-xl text-xs focus:ring-2 focus:ring-rose-500 bg-white text-slate-800 font-medium"
        />
        <label className={`px-3 py-2 rounded-xl bg-gradient-to-r from-amber-400 to-amber-300 hover:brightness-105 text-rose-950 font-bold text-xs flex items-center gap-1.5 cursor-pointer shrink-0 shadow-2xs ${uploading ? 'opacity-50 pointer-events-none' : ''}`}>
      ...
      {value && (
        <div className="relative h-36 w-full mt-2 rounded-xl overflow-hidden border-2 border-rose-300/80 bg-slate-900 flex items-center justify-center group shadow-xs">
          ...
          />
          <div className="absolute inset-0 bg-slate-950/40 opacity-0 group-hover:opacity-100 transition flex items-center justify-center text-white text-[11px] font-bold pointer-events-none">
            Xem trước ảnh đại diện (Hiển thị thực tế)
          </div>
        </div>
      )}
    </div>
