...
export const NotificationAdminView: React.FC<NotificationAdminViewProps> = ({
  ...
  const [viewDetailItem, setViewDetailItem] = useState<NotificationItem | null>(null);
  ...
  return (
    <div className="space-y-6">
                    ...
                    filteredList.map(item => {
                      ...
                      return (
                        <tr key={item.id} className={`hover:bg-slate-50/80 transition-colors ${isSelected ? 'bg-blue-50/50' : ''}`}>
                          ...
                          <td className="py-3.5 px-4 min-w-[280px] max-w-lg">
                            <div className="flex flex-col gap-1.5 min-w-0 w-full">
                              <div className="font-bold text-slate-900 leading-snug break-words">
                                {item.title}
                              </div>
                              <div className="text-slate-700 text-xs leading-relaxed break-words whitespace-pre-wrap max-h-36 overflow-y-auto pr-1.5 p-2.5 bg-slate-50/80 rounded-xl border border-slate-200/80 font-normal">
                                {item.body}
                              </div>
                              {item.action_url && (
                                <a
                                  ...
                                  className="inline-flex items-center gap-1 text-[11px] text-blue-600 font-bold hover:underline mt-0.5 break-all"
                                >
                                  <span className="break-all">{item.action_url}</span>
                                  <ExternalLink className="w-3 h-3 shrink-0" />
                                </a>
                              )}
                            </div>
                          </td>
                          <td className="py-3.5 px-4 font-semibold capitalize text-slate-700 whitespace-nowrap">
                            {item.category}
                          </td>
      ...
      {viewDetailItem && (
        <div className="fixed inset-0 z-50 bg-slate-950/70 backdrop-blur-xs flex items-center justify-center p-3 sm:p-6 min-w-0">
          <div className="bg-white rounded-3xl max-w-2xl w-full max-h-[90vh] flex flex-col shadow-2xl border border-slate-200 overflow-hidden animate-in fade-in zoom-in-95 min-w-0">
            ...
            <div className="p-4 sm:p-5 bg-gradient-to-r from-blue-900 via-indigo-900 to-slate-900 text-white flex items-center justify-between shrink-0">
              <div className="flex items-center gap-3 min-w-0">
                <div className="p-2.5 bg-white/15 text-cyan-200 rounded-2xl border border-white/20 shrink-0">
                  <Bell className="w-6 h-6" />
                </div>
                <div className="min-w-0">
                  <h3 className="font-black text-sm sm:text-base text-white truncate">Chi Tiết Bản Tin Thông Báo Quản Trị</h3>
                  <p className="text-xs text-blue-200 font-mono truncate">
                    Mã bản ghi: <span className="font-bold">{viewDetailItem.id}</span>
                  </p>
                </div>
              </div>
              <button
            ...
            {/* Scrollable Content Body with overflow-y-auto and break-words */}
            <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-4 min-w-0">
              <div className="space-y-1">
                <label className="text-[10px] font-black uppercase tracking-wider text-slate-400">Tiêu đề thông báo</label>
              ...
              <div className="space-y-1">
                <label className="text-[10px] font-black uppercase tracking-wider text-slate-400">Nội dung chi tiết</label>
                <div className="p-4 bg-slate-50 rounded-2xl border border-slate-200 text-xs sm:text-sm text-slate-800 leading-relaxed font-medium whitespace-pre-wrap break-words max-h-72 overflow-y-auto pr-2">
                  {viewDetailItem.body}
                </div>
              </div>

              {viewDetailItem.action_url && (
                <div className="space-y-1">
                  <label className="text-[10px] font-black uppercase tracking-wider text-slate-400">Đường dẫn liên kết / Tác vụ đính kèm</label>
                  <div>
                    <a
                      href={viewDetailItem.action_url}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="inline-flex items-center gap-2 px-3.5 py-2.5 rounded-xl bg-blue-50 text-blue-700 font-bold text-xs border border-blue-200 hover:bg-blue-100 transition break-all"
                    >
                      <span className="break-all">{viewDetailItem.action_url}</span>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-2 text-xs">
                <div className="p-3 bg-slate-50 rounded-2xl border border-slate-200 space-y-1">
                  ...
                  <span className="font-bold text-slate-800 capitalize">{viewDetailItem.category}</span>
                </div>
                <div className="p-3 bg-slate-50 rounded-2xl border border-slate-200 space-y-1">
                  <span className="text-[10px] text-slate-400 font-bold uppercase block">Mức độ ưu tiên</span>
                  <span className={`inline-block px-2 py-0.5 rounded text-[10px] font-black ${
                    viewDetailItem.priority === 'CRITICAL' ? 'bg-rose-600 text-white' :
                    viewDetailItem.priority === 'URGENT' ? 'bg-amber-500 text-white' : 'bg-blue-100 text-blue-800'
                  }`}>
                    {viewDetailItem.priority}
                  </span>
                </div>
                <div className="p-3 bg-slate-50 rounded-2xl border border-slate-200 space-y-1 col-span-1 sm:col-span-1">
                  <span className="text-[10px] text-slate-400 font-bold uppercase block">Thời gian phát hành</span>
                  <span className="font-bold text-slate-800 font-mono text-[11px]">
                    {viewDetailItem.created_at ? new Date(viewDetailItem.created_at).toLocaleString('vi-VN') : 'Vừa xong'}
                  </span>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
                <div className="p-3 bg-slate-50 rounded-2xl border border-slate-200 space-y-1">
                  <span className="text-[10px] text-slate-400 font-bold uppercase block">Đối tượng nhận</span>
                  <span className="font-bold text-slate-800 break-words">
                    {viewDetailItem.target_type === 'ALL' ? 'Tất cả người dùng' :
                     viewDetailItem.target_type === 'AUTHENTICATED' ? 'Thành viên đã đăng nhập' :
                     viewDetailItem.target_type === 'ROLE' ? `Vai trò: ${viewDetailItem.target_roles?.join(', ')}` : viewDetailItem.target_type}
                  </span>
                </div>
                <div className="p-3 bg-slate-50 rounded-2xl border border-slate-200 space-y-1">
                  <span className="text-[10px] text-slate-400 font-bold uppercase block">Kênh phát tin</span>
                  <span className="font-bold text-indigo-700">
                    {viewDetailItem.channels?.join(', ') || 'IN_APP, WEB_PUSH'}
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="p-4 bg-slate-50 border-t border-slate-200 flex items-center justify-between shrink-0">
              <button
                onClick={() => {
                  handleDeleteSingle(viewDetailItem.id);
                  setViewDetailItem(null);
                }}
                className="px-4 py-2.5 rounded-xl bg-rose-50 hover:bg-rose-100 text-rose-700 font-bold text-xs flex items-center gap-1.5 transition cursor-pointer"
              >
